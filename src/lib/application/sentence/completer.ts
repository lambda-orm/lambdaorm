import {
	Sentence, EntityMapping, Field, Filter, Join, SchemaError, SentenceInclude, Insert, Update, PropertyMapping,
...
export class SentenceCompleter {
	...
	public complete (mapping: IMappingConfigService, view: IViewConfigService, sentence: Sentence) {
	...
	private solveJoin (sentence: Sentence, mapping: IMappingConfigService) {
	...
	private solveProperties (operand:Operand, mapping: IMappingConfigService, view:IViewConfigService) {
		for (const i in operand.children) {
			const child = operand.children[i]
