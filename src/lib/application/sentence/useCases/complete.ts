import {
	...
	PropertyView, SentenceAction
} from '../../../domain'
import { IExpressions, Operand, OperandType } from '3xpr'
import { Type } from 'typ3s'
import { MappingConfigService, ViewConfigService } from '../../'
...
export class SentenceCompleter {
	...
	private solveJoin (sentence: Sentence, mapping: MappingConfigService) {
	...
	private solveProperties (operand:Operand, mapping: MappingConfigService, view:ViewConfigService) {
		for (const i in operand.children) {
			const child = operand.children[i]
