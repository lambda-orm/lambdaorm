...
export class QueryInsertConditionalExecutor extends QueryInsertExecutor {
	public async insertConditional (query: Query, data: Data, mapping: MappingConfigService, dialect: DialectService, connection: Connection): Promise<any> {
		...
		// insert main entity
		const insertId = await connection.insertConditional(mapping, dialect, query, data)
		if (autoIncrement) {
