...
import { RuleDataSource, Query, Delta, Index, DataSource, Relation, Entity, EntityMapping, PropertyMapping, SentenceInfo } from '../model'
...
export class DDLBuilder {
	...
	public truncate (entities: string[]):Query[] {
		const queries:Query[] = []
		const stage = this.schema.stage.get(this.stage)
		for (const k in stage.dataSources) {
			const ruleDataSource = stage.dataSources[k]
