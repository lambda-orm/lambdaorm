...
describe('group', () => {
	...
	test('getInfo', () => {
		expect(orm.getInfo('Countries.filter(p=> p.region == "Americas").map(p=> {result:count(1)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT COUNT(1) AS result FROM Countries c  WHERE c.region = 'Americas' ","children":[]})
		expect(orm.getInfo('Countries.map(p=> {region:p.region,countries:count(1)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.region AS region, COUNT(1) AS countries FROM Countries c  GROUP BY c.region ","children":[]})
		expect(orm.getInfo('Countries.map(p=> {region:p.region,max:max(p.latitude)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.region AS region, MAX(c.latitude) AS max FROM Countries c  GROUP BY c.region ","children":[]})
		expect(orm.getInfo('Countries.map(p=> {region:p.region,min:min(p.latitude)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.region AS region, MIN(c.latitude) AS min FROM Countries c  GROUP BY c.region ","children":[]})
		expect(orm.getInfo('Countries.map(p=> {region:p.region,avg:avg(p.latitude)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.region AS region, AVG(c.latitude) AS avg FROM Countries c  GROUP BY c.region ","children":[]})
	})
})
