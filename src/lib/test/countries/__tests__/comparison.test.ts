...
describe('comparison', () => {
	...
	test('getInfo', () => {
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> p.name)',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.name AS name FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 === "BRA").map(p=> p.name)',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.name AS name FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.latitude < -9 && p.latitude > -11 && p.longitude == -55 ).first(p=> p.name)',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.name AS name FROM Countries c  WHERE (c.latitude < -9 AND (c.latitude > -11 AND c.longitude = -55)) ORDER BY c.name asc  LIMIT 0,1 "})
		expect(orm.plan('Countries.filter(p=> between(p.latitude,-11,-9) && p.longitude == -55 ).first(p=> p.name)',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.name AS name FROM Countries c  WHERE (c.latitude BETWEEN -11 AND -9 AND c.longitude = -55) ORDER BY c.name asc  LIMIT 0,1 "})
		expect(orm.plan('Countries.filter(p=> in(p.iso3,["BRA","ARG"])).map(p=> p.name)',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT c.name AS name FROM Countries c  WHERE  c.iso3 IN ('BRA', 'ARG') "})
	})
})
