...
describe('nullable', () => {
	...
	test('getInfo', () => {
		expect(orm.getInfo('States.filter(p=> isNull(p.latitude)).map(p=> count(1))',{ stage: 'stage1' })).toStrictEqual({"entity":"States","dialect":"PostgreSQL","source":"dataSource2","sentence":"SELECT COUNT(1) FROM TBL_STATES s  WHERE (s.LATITUDE IS NULL) ","children":[]})
		expect(orm.getInfo('States.filter(p=> isNotNull(p.latitude)).map(p=> count(1))',{ stage: 'stage1' })).toStrictEqual({"entity":"States","dialect":"PostgreSQL","source":"dataSource2","sentence":"SELECT COUNT(1) FROM TBL_STATES s  WHERE (s.LATITUDE IS NOT NULL) ","children":[]})
		expect(orm.getInfo('States.filter(p=> nvl(p.latitude,-100)== -100).map(p=> count(1))',{ stage: 'stage1' })).toStrictEqual({"entity":"States","dialect":"PostgreSQL","source":"dataSource2","sentence":"SELECT COUNT(1) FROM TBL_STATES s  WHERE (CASE WHEN s.LATITUDE IS NOT NULL THEN s.LATITUDE ELSE -100 END) = -100 ","children":[]})
		expect(orm.getInfo('Countries.filter(p=> p.iso3 == "CIV" ).map(p=> {native: nvl(p.native,"???")})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT IFNULL(c.native,'???') AS native FROM Countries c  WHERE c.iso3 = 'CIV' ","children":[]})
		expect(orm.getInfo('Countries.filter(p=> p.iso3 == "CIV" ).map(p=> {native: nvl2(p.native,"is not null","is null")})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT (CASE WHEN c.native IS NOT NULL THEN 'is not null' ELSE 'is null' END) AS native FROM Countries c  WHERE c.iso3 = 'CIV' ","children":[]})
	})
})
