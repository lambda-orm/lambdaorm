...
describe('string', () => {
	...
	test('getInfo', () => {
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: lower(p.subregion) })',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT LOWER(c.subregion) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: upper(p.subregion) })',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT UPPER(c.subregion) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: concat(p.region," ",p.subregion) })',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT CONCAT(CONCAT(c.region,' '),c.subregion) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: lpad(p.region,12,"_") })',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT LPAD(c.region,12,'_') AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: rpad(p.region,12,"_") })',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT RPAD(c.region,12,'_') AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: chr(68)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT CHAR(68 USING ASCII) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: ltrim("  a  ")})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT LTRIM('  a  ') AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: rtrim("  a  ")})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT RTRIM('  a  ') AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: replace(p.region,"a","*")})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT REPLACE(c.region,'a','*') AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: mask(p.subregion)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT Concat(LEFT(c.subregion,3),'***',RIGHT(c.subregion,3)) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: substr(p.subregion,1,3)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT SUBSTR(c.subregion,1,3) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: substring(p.subregion,1,3)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT SUBSTR(c.subregion,1,3) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: strCount(p.subregion,"a")})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT (LENGTH(c.subregion)-LENGTH(REPLACE(c.subregion,'a',''))) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
		expect(orm.plan('Countries.filter(p=> p.iso3 == "BRA").map(p=> {result: toString(p.latitude)})',{ stage: 'stage1' })).toStrictEqual({"entity":"Countries","dialect":"MySQL","source":"dataSource1","sentence":"SELECT CONVERT(c.latitude, CHAR) AS result FROM Countries c  WHERE c.iso3 = 'BRA' "})
	})
})
