...
describe('Execute', () => {
	...
	test('insert 3', async () => {
		...
		const expected = {"data":{"entity":{"name":"Beverages22","description":"Soft drinks, coffees, teas, beers, and ales"},"id":14}}
		const MySQLResult =  await orm.execute(expression, data,{stage:'MySQL'})
		expect(expected).toEqual(MySQLResult)
		const MariaDBResult =  await orm.execute(expression, data,{stage:'MariaDB'})
		expect(expected).toEqual(MariaDBResult)
	...
	test('insert 4', async () => {
		const expression = 'Categories.insert(entity)'
		const expected = {"data":{"entity":{"name":"Beverages23","description":"Soft drinks, coffees, teas, beers, and ales"},"id":16}}
		const MySQLResult =  await orm.execute(expression, data,{stage:'MySQL'})
		expect(expected).toEqual(MySQLResult)
		const MariaDBResult =  await orm.execute(expression, data,{stage:'MariaDB'})
		expect(expected).toEqual(MariaDBResult)
