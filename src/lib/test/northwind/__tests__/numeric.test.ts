...
describe('Sentences', () => {
	test('function abs', async () => {
		...
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":{ \"$multiply\" :[\"$UnitPrice\",-1] }, \"result\":{ \"$round\" :[{ \"$abs\" :{ \"$multiply\" :[\"$UnitPrice\",-1] } },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
	...
	test('function acos', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":0.25, \"result\":{ \"$round\" :[{ \"$acos\" :0.25 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function asin', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":0.25, \"result\":{ \"$round\" :[{ \"$asin\" :0.25 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function atan', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":0.25, \"result\":{ \"$round\" :[{ \"$atan\" :0.25 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function atan2', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":0.5, \"result\":{ \"$round\" :[{ \"$atan2\":[0.25,1] },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function ceil', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":25.75, \"result\":{ \"$round\" :[{ \"$ceil\" :25.75 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function cos', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":2, \"result\":{ \"$round\" :[{ \"$cos\" :2 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function exp', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":1, \"result\":{ \"$round\" :[{ \"$exp\" :1 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function floor', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":25.75, \"result\":{ \"$round\" :[{ \"$floor\" :25.75 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function ln', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":2, \"result\":{ \"$round\" :[{ \"$ln\" :2 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function log', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"m\":10, \"n\":20, \"result\":{ \"$round\" :[{ \"$log\":[10,20] },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function round', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":135.375, \"result\":{ \"$round\" :[135.375,2] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function sign', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":255.5, \"result\":{ \"$round\" :[{\"$cond\":{\"if\":{\"$gt\":[255.5,0]},\"then\":1,\"else\":{\"if\":{\"$lt\":[255.5,0]},\"then\":-1,\"else\":0}}},10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function tan', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":1.75, \"result\":{ \"$round\" :[{ \"$tan\" :1.75 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('function trunc', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":135.375, \"result\":{ \"$round\" :[{ \"$trunc\" :135.375 },10] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
