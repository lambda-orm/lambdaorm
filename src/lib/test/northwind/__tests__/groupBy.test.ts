...
describe('Sentences', () => {
	test('groupBy 1', async () => {
		...
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$group\" :{ \"_id\": 0 , \"maxPrice\":{ \"$max\" :\"$UnitPrice\" } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
	...
	test('groupBy 2', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$group\" :{ \"_id\": 0 , \"minPrice\":{ \"$min\" :\"$UnitPrice\" } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('groupBy 3', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$group\" :{ \"_id\": 0 , \"total\":{ \"$sum\" :\"$UnitPrice\" } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('groupBy 4', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$group\" :{ \"_id\": 0 , \"average\":{ \"$round\" :[{ \"$avg\" :\"$UnitPrice\" },4] } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('groupBy 5', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$group\" :{ \"_id\": 0 , \"count\":{ \"$sum\" : 1 } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('groupBy 6', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$group\" :{ \"_id\":{ \"category\":\"$CategoryID\" } , \"largestPrice\":{ \"$max\" :\"$UnitPrice\" } }}, { \"$project\" :{ \"_id\": 0 , \"category\":\"$_id.category\" , \"largestPrice\":\"$largestPrice\" }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('groupBy 8', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$match\" : { \"_id\":{{id}} } }, { \"$project\" :{ \"_id\": 0 , \"name\":\"$ProductName\", \"source\":\"$UnitPrice\", \"result\":{ \"$abs\" :\"$UnitPrice\" } }} , { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('groupBy 9', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Products","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$lookup\" :{ \"from\": \"Categories\", \"localField\": \"CategoryID\" , \"foreignField\": \"_id\", \"as\": \"c\" }}, { \"$group\" :{ \"_id\":{ \"category\":{ \"$arrayElemAt\": [\"$c.CategoryName\", 0] } } , \"largestPrice\":{ \"$max\" :\"$UnitPrice\" } }}, { \"$project\" :{ \"_id\": 0 , \"category\":\"$_id.category\" , \"largestPrice\":\"$largestPrice\" }} , { \"$match\" : { { \"$max\" :\"UnitPrice\" }: { \"$gt\": 100 } } }, { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	...
	test('query 10', async () => {
		...
		let SqlServer = orm.getInfo(expression,{stage:'SqlServer'})
		expect(SqlServerExpected).toStrictEqual(SqlServer)
		const MongoDBExpected = {"entity":"Orders.details","dialect":"MongoDB","source":"MongoDB","sentence":"[{ \"$unwind\" : \"$\\\"Order Details\\\"\" }, { \"$replaceRoot\": { \"newRoot\": \"$\\\"Order Details\\\"\" } }, { \"$group\" :{ \"_id\": 0 , \"subTotal\":{ \"$sum\" :{ \"$multiply\" :[{ \"$multiply\" :[{ \"$multiply\" :[\"$UnitPrice\",\"$Quantity\"] },{ \"$subtract\" :[1,{ \"$divide\" :[\"$Discount\",100] }] }] },100] } } }} , { \"$sort\" :{ \"subTotal\":1 } }, { \"$project\": { \"_id\": 0 } }]","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
