...
describe('Metadata', () => {
	...
	test('update 4', async () => {
		...
		const parametersExpected:any = [{"name":"entity.name","type":"any"},{"name":"entity.id","type":"any"}]
		const metadataExpected :any= {"classtype":"Sentence","pos":{"ln":0,"col":51},"name":"update","children":[],"type":"any","entity":"Orders","columns":[{"name":"name","type":"string"}],"parameters":[{"name":"entity.name","type":"any"},{"name":"entity.id","type":"any"}],"constraints":[],"values":[],"defaults":[],"clause":"update","alias":"o"}
		const constraintsExpected :any= {"entity":"Orders","constraints":[]}
		const model = orm.model(expression)
		const parameters = orm.parameters(expression)
		const constraints = orm.constraints(expression)
	...
	test('update 6', async () => {
		...
		const parametersExpected:any = [{"name":"entity.name","type":"any"},{"name":"entity.id","type":"any"},{"name":"details","type":"Orders.details","children":[{"name":"unitPrice","type":"decimal"},{"name":"quantity","type":"decimal"},{"name":"discount","type":"decimal"},{"name":"orderId","type":"integer"},{"name":"productId","type":"integer"}]}]
	...
	test('update 7', async () => {
		...
		const parametersExpected:any = [{"name":"entity.name","type":"any"},{"name":"entity.id","type":"any"},{"name":"details","type":"Orders.details","children":[{"name":"orderId","type":"integer"},{"name":"productId","type":"integer"}]}]
...
describe('Sentences', () => {
	...
	test('update 3', async () => {
		...
		let Oracle = orm.getInfo(expression,{stage:'Oracle'})
		expect(OracleExpected).toStrictEqual(Oracle)
		const MongoDBExpected = {"entity":"Orders","dialect":"MongoDB","source":"MongoDB","sentence":"{\"set\":\"{ \\\"ShipPostalCode\\\":{{postalCode}} }\",\"filter\":\"{}\"}","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	})
	test('update 4', async () => {
		...
		let Oracle = orm.getInfo(expression,{stage:'Oracle'})
		expect(OracleExpected).toStrictEqual(Oracle)
		const MongoDBExpected = {"entity":"Orders","dialect":"MongoDB","source":"MongoDB","sentence":"{\"set\":\"{ \\\"ShipName\\\":{{entity_name}} }\",\"filter\":\"{ \\\"_id\\\":{{entity_id}} }\"}","children":[]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	})
	test('update 5', async () => {
	...
	test('update 6', async () => {
		const expression = 'Orders.update(() => ({ name: entity.name })).include(p => p.details.update(p => p)).filter(p => p.id === entity.id)'
		const MySQLExpected = {"entity":"Orders","dialect":"MySQL","source":"MySQL","sentence":"UPDATE Orders o SET ShipName = ? WHERE o.OrderID = ? ","children":[{"entity":"Orders.details","dialect":"MySQL","source":"MySQL","sentence":"UPDATE `Order Details` o1 SET UnitPrice = ?,Quantity = ?,Discount = ? WHERE (o1.OrderID = ? AND o1.ProductID = ?) ","children":[]}]}
		let MySQL = orm.getInfo(expression,{stage:'MySQL'})
		expect(MySQLExpected).toStrictEqual(MySQL)
		const MariaDBExpected = {"entity":"Orders","dialect":"MariaDB","source":"MariaDB","sentence":"UPDATE Orders o SET ShipName = ? WHERE o.OrderID = ? ","children":[{"entity":"Orders.details","dialect":"MariaDB","source":"MariaDB","sentence":"UPDATE `Order Details` o1 SET UnitPrice = ?,Quantity = ?,Discount = ? WHERE (o1.OrderID = ? AND o1.ProductID = ?) ","children":[]}]}
		let MariaDB = orm.getInfo(expression,{stage:'MariaDB'})
		expect(MariaDBExpected).toStrictEqual(MariaDB)
		const PostgreSQLExpected = {"entity":"Orders","dialect":"PostgreSQL","source":"PostgreSQL","sentence":"UPDATE Orders o SET ShipName = $1 WHERE o.OrderID = $2 ","children":[{"entity":"Orders.details","dialect":"PostgreSQL","source":"PostgreSQL","sentence":"UPDATE \"Order Details\" o1 SET UnitPrice = $1,Quantity = $2,Discount = $3 WHERE (o1.OrderID = $4 AND o1.ProductID = $5) ","children":[]}]}
		let PostgreSQL = orm.getInfo(expression,{stage:'PostgreSQL'})
		expect(PostgreSQLExpected).toStrictEqual(PostgreSQL)
	...
	test('update 7', async () => {
		...
		let Oracle = orm.getInfo(expression,{stage:'Oracle'})
		expect(OracleExpected).toStrictEqual(Oracle)
		const MongoDBExpected = {"entity":"Orders","dialect":"MongoDB","source":"MongoDB","sentence":"{\"set\":\"{ \\\"ShipName\\\":{{entity_name}} }\",\"filter\":\"{ \\\"_id\\\":{{entity_id}} }\"}","children":[{"entity":"Orders.details","dialect":"MongoDB","source":"MongoDB","sentence":"{\"set\":\"{ \\\"UnitPrice\\\":\\\"UnitPrice\\\",\\\"ProductID\\\":\\\"ProductID\\\" }\",\"filter\":\"{ \\\"$and\\\" :[{ \\\"OrderID\\\":{{orderId}} },{ \\\"ProductID\\\":{{productId}} }] }\"}","children":[]}]}
		let MongoDB = orm.getInfo(expression,{stage:'MongoDB'})
		expect(MongoDBExpected).toStrictEqual(MongoDB)
	})
	test('update 8', async () => {
