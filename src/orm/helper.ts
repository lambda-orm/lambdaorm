...
export class Helper {
	...
	public static deltaWithSimpleArrays (current:any, old?:any):Delta {
		...
		if (current === undefined || current === null) { throw new Error('current value can\'t empty') }
		for (const name in current) {
			const currentValue = current[name]
			if (old === undefined || old === null) { delta.new.push({ name: name, new: currentValue }) } else {
				...
				if (oldValue === undefined) {
					...
					if (change) { delta.changed.push({ name: name, new: currentValue, old: oldValue, delta: objectDelta }) } else { delta.unchanged.push({ name: name, value: oldValue }) }
					// const objectDelta = Helper.deltaWithSimpleArrays(currentValue,oldValue)
					// const change = objectDelta.changed.length + objectDelta.remove.length + objectDelta.new.length > 0
