...
import { Query, Delta, Index, Datastore, Relation, Entity, EntityMapping, PropertyMapping, IEvaluator } from '../model'
...
export class DDLBuilder {
	...
	public async drop (entities: string[]): Promise<Query[]> {
		...
		for (const p in sortedEntities) {
			...
			const datastore = await this.getDatastore(entityName)
			...
			if (entity.relations) {
				for (const q in entity.relations) {
					...
					if (relation.type === 'oneToMany' || relation.type === 'oneToOne') {
						const query = this.builder(datastore).dropFk(entity, relation)
						queries.push(query)
					}
				}
			}
		...
		for (const i in sortedEntities) {
			...
			if (entity.indexes) {
				for (const j in entity.indexes) {
					...
					const query = this.builder(datastore).dropIndex(entity, index)
					queries.push(query)
				}
			}
			const query = this.builder(datastore).dropEntity(entity)
			queries.push(query)
		}
		return queries
	}
	...
	public async truncate (entities: string[]):Promise<Query[]> {
		...
		for (const i in entities) {
			...
			const entity = this.model.getEntity(entityName) as Entity
			const datastore = await this.getDatastore(entity.name)
			const query = this.builder(datastore).truncateEntity(entity)
			queries.push(query)
		}
		return queries
	}
	...
	public async sync (delta:Delta):Promise<Query[]> {
		...
		for (const p in delta.changed) {
			...
			const datastore = await this.getDatastore(entityChanged.name)
			for (const q in entityChanged.delta.changed) {
				...
				if (changed.name === 'primaryKey') {
					...
					for (const n in changed.delta.remove) {
						const query = this.builder(datastore).dropPk(entityChanged.old)
						queries.push(query)
					}
					// eslint-disable-next-line @typescript-eslint/no-unused-vars
				...
				if (changed.name === 'uniqueKey') {
					if (changed.delta) {
						// eslint-disable-next-line @typescript-eslint/no-unused-vars
						for (const n in changed.delta.remove) {
							const query = this.builder(datastore).dropUk(entityChanged.old)
							queries.push(query)
						}
						// eslint-disable-next-line @typescript-eslint/no-unused-vars
						for (const c in changed.delta.changed) {
			...
			for (const q in entityChanged.delta.changed) {
				...
				if (changed.name === 'index') {
					...
					for (const c in changed.delta.changed) {
						...
						const query = this.builder(datastore).dropIndex(entityChanged.new, oldIndex)
						queries.push(query)
					}
					for (const r in changed.delta.remove) {
						const removeIndex = changed.delta.remove[r].old as Index
						const query = this.builder(datastore).dropIndex(entityChanged.new, removeIndex)
						queries.push(query)
					}
				}
				if (changed.name === 'relation') {
					...
					for (const c in changed.delta.changed) {
						...
						if (this.changeRelation(oldRelation, newRelation)) {
							if (oldRelation.type === 'oneToMany' || oldRelation.type === 'oneToOne') {
								const query = this.builder(datastore).dropFk(entityChanged.new, oldRelation)
								queries.push(query)
							}
						}
					}
					for (const r in changed.delta.remove) {
						...
						const query = this.builder(datastore).dropFk(entityChanged.new, removeRelation)
						queries.push(query)
					}
				}
			}
		...
		for (const name in delta.remove) {
			...
			const datastore = await this.getDatastore(removeEntity.name)

			if (removeEntity.indexes) {
				for (const i in removeEntity.indexes) {
					const index = removeEntity.indexes[i]
					const query = this.builder(datastore).dropIndex(removeEntity, index)
					queries.push(query)
				}
			}
			const query = this.builder(datastore).dropEntity(removeEntity)
			queries.push(query)
		}
		// create tables
		for (const name in delta.new) {
			const newEntity = delta.new[name].new as EntityMapping
			const datastore = await this.getDatastore(newEntity.name)
			const query = this.builder(datastore).createEntity(newEntity)
			queries.push(query)
		}
			...
			for (const q in entityChanged.delta.changed) {
				...
				if (changed.name === 'property') {
					...
					for (const n in changed.delta.new) {
						...
						const query = this.builder(datastore).addColumn(entityChanged.new, newProperty)
						queries.push(query)
					}
					for (const n in changed.delta.changed) {
						...
						if (newProperty.mapping === oldProperty.mapping) {
							const query = this.builder(datastore).alterColumn(entityChanged.new, newProperty)
							queries.push(query)
						}
					}
				}
			...
			for (const q in entityChanged.delta.changed) {
				...
				if (changed.name === 'property') {
					...
					for (const n in changed.delta.remove) {
						...
						const query = this.builder(datastore).dropColumn(entityChanged.old, oldProperty)
						queries.push(query)
					}
				}
			}
			...
			for (const q in entityChanged.delta.changed) {
				...
				if (changed.name === 'primaryKey') {
					...
					for (const n in changed.delta.new) {
						...
						const query = this.builder(datastore).addPk(entityChanged.new, newPrimaryKey)
						queries.push(query)
					}
					for (const c in changed.delta.changed) {
						const changePrimaryKey = changed.delta.changed[c].new as string[]
						const query = this.builder(datastore).addPk(entityChanged.new, changePrimaryKey)
						queries.push(query)
					}
				}
				if (changed.name === 'uniqueKey') {
					if (changed.delta) {
						for (const n in changed.delta.new) {
							...
							const query = this.builder(datastore).addUk(entityChanged.new, newUniqueKey)
							queries.push(query)
						}
						for (const c in changed.delta.changed) {
							const chanegUniqueKey = changed.delta.changed[c].new as string[]
							const query = this.builder(datastore).addUk(entityChanged.new, chanegUniqueKey)
							queries.push(query)
						}
					}
				}
			...
			for (const q in entityChanged.delta.changed) {
				...
				if (changed.name === 'index') {
					...
					for (const n in changed.delta.new) {
						...
						const query = this.builder(datastore).createIndex(entityChanged.new, newIndex)
						queries.push(query)
					}
					for (const c in changed.delta.changed) {
						const changeIndex = changed.delta.changed[c].new as Index
						const query = this.builder(datastore).createIndex(entityChanged.new, changeIndex)
						queries.push(query)
					}
					// for(const r in changed.delta.remove){
					...
					if (changed.delta) {
						for (const n in changed.delta.new) {
							...
							if (newRelation.type === 'oneToMany' || newRelation.type === 'oneToOne') {
								const query = this.builder(datastore).addFk(entityChanged.new, newRelation)
								queries.push(query)
							}
						}
						for (const c in changed.delta.changed) {
		...
		for (const name in delta.new) {
			...
			if (newEntity.indexes) {
				for (const i in newEntity.indexes) {
					...
					const query = this.builder(datastore).createIndex(newEntity, index)
					queries.push(query)
				}
			}
			if (newEntity.relations) {
				for (const p in newEntity.relations) {
					...
					if (relation.type === 'oneToMany' || relation.type === 'oneToOne') {
						const query = this.builder(datastore).addFk(newEntity, relation)
						queries.push(query)
					}
				}
			}
	...
	private async getDatastore (entity: string): Promise<Datastore> {
	...
	private builder (datastore: Datastore): LanguageDDLBuilder {
		return this.languageManager.ddlBuilder(datastore)
	}
