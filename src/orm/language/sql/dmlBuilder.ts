...
export class SqlDMLBuilder extends LanguageDMLBuilder {
	public build (sentence: Sentence, datastore: string, metadata: DialectMetadata): Query {
	...
	private buildSentence (sentence:Sentence, metadata:DialectMetadata):string {
		...
		} else if (insert) text = this.buildInsert(insert, sentence.entity, metadata)
	...
	private buildInsert (operand:Insert, entity:string, metadata:DialectMetadata):string {
		let template = metadata.dml(operand.clause)
		const templateColumn = metadata.other('column')
		const fields:string[] = []
		const values: any[] = []
		const autoincrement = this.config.model.getAutoincrement(entity)
