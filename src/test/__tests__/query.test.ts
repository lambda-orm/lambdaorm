...
describe('Complete Expression', () => {
	...
	test('query 14', () => {
		...
		const expected = 'Products.sort(p=>asc(p.id)).page(1,1).map(p=>{category:p.category.name,name:p.name,quantity:p.quantity,inStock:p.inStock})'
		const target = orm.normalize(source)
		expect(expected).toBe(target)
	})
	test('query 15', () => {
...
describe('Sentences', () => {
	...
	test('query 14', async () => {
		const expression = 'Products.first(p=>{category:p.category.name,name:p.name,quantity:p.quantity,inStock:p.inStock})'
		const MySQLExpected = {"entity":"Products","dialect":"MySQL","dataSource":"MySQL","sentence":"SELECT c.CategoryName AS category, p.ProductName AS name, p.QuantityPerUnit AS quantity, p.UnitsInStock AS inStock FROM Products p INNER JOIN Categories c ON c.CategoryID = p.CategoryID ORDER BY p.ProductID asc  LIMIT 0,1 ","children":[]}
		let MySQL = orm.sentence(expression,{stage:'MySQL'})
		expect(MySQLExpected).toStrictEqual(MySQL)
		const MariaDBExpected = {"entity":"Products","dialect":"MariaDB","dataSource":"MariaDB","sentence":"SELECT c.CategoryName AS category, p.ProductName AS name, p.QuantityPerUnit AS quantity, p.UnitsInStock AS inStock FROM Products p INNER JOIN Categories c ON c.CategoryID = p.CategoryID ORDER BY p.ProductID asc  LIMIT 0,1 ","children":[]}
		let MariaDB = orm.sentence(expression,{stage:'MariaDB'})
		expect(MariaDBExpected).toStrictEqual(MariaDB)
