...
describe('Complete Expression', () => {
	test('bulkInsert 1', () => {
		const source = 'northwind_1.Categories.bulkInsert()'
		const expected = 'northwind_1.Categories.bulkInsert({name:name,description:description})'
		const target = orm.expression(source).complete('northwind')
		expect(expected).toBe(target)
	})
	test('bulkInsert 2', () => {
		const source = 'northwind_1.Orders.bulkInsert().include(p=>p.details)'
		const expected = 'northwind_1.Orders.bulkInsert({customerId:customerId,employeeId:employeeId,orderDate:orderDate,requiredDate:requiredDate,shippedDate:shippedDate,shipViaId:shipViaId,freight:freight,name:name,address:address,city:city,region:region,postalCode:postalCode,country:country}).include(p=>p.details.bulkInsert({orderId:orderId,productId:productId,unitPrice:unitPrice,quantity:quantity,discount:discount}))'
		const target = orm.expression(source).complete('northwind')
		expect(expected).toBe(target)
	})
})
