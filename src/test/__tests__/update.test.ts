...
describe('Complete Expression', () => {
	...
	test('update 3', () => {
		const source = 'northwind_1.Orders.updateAll(() => ({ postalCode: postalCode }))'
		const expected = 'northwind_1.Orders.update(=>{postalCode:postalCode})'
		const target = orm.expression(source).complete('northwind')
		expect(expected).toBe(target)
	})
	test('update 4', () => {
	...
	test('update 5', () => {
		const source = 'northwind_1.Orders.update(() => ({ name: entity.name })).include(p => p.details).filter(p => p.id === entity.id)'
		const expected = 'northwind_1.Orders.update(=>{name:entity.name}).include(p=>p.details.filter(p=>((p.orderId==obj.orderId)&&(p.productId==obj.productId))).update({orderId:orderId,productId:productId,unitPrice:unitPrice,quantity:quantity,discount:discount})).filter(p=>(p.id===entity.id))'
		const target = orm.expression(source).complete('northwind')
		expect(expected).toBe(target)
	})
	test('update 6', () => {
		const source = 'northwind_1.Orders.update(() => ({ name: entity.name })).include(p => p.details.update(p => p)).filter(p => p.id === entity.id)'
		const expected = 'northwind_1.Orders.update(=>{name:entity.name}).include(p=>p.details.filter(p=>((p.orderId==orderId)&&(p.productId==productId))).update(p=>{unitPrice:p.unitPrice,quantity:p.quantity,discount:p.discount})).filter(p=>(p.id===entity.id))'
		const target = orm.expression(source).complete('northwind')
		expect(expected).toBe(target)
	})
	test('update 7', () => {
		const source = 'northwind_1.Orders.update(() => ({ name: entity.name })).include(p => p.details.update(p => ({ unitPrice: p.unitPrice, productId: p.productId }))).filter(p => p.id === entity.id)'
		const expected = 'northwind_1.Orders.update(=>{name:entity.name}).include(p=>p.details.filter(p=>((p.orderId==orderId)&&(p.productId==productId))).update(p=>{unitPrice:p.unitPrice,productId:p.productId})).filter(p=>(p.id===entity.id))'
		const target = orm.expression(source).complete('northwind')
		expect(expected).toBe(target)
	})
	test('update 8', () => {
...
describe('Sentences', () => {
	...
	test('update 3', async () => {
		const expression = 'northwind_1.Orders.updateAll(() => ({ postalCode: postalCode }))'
		const mariadbExpected = 'UPDATE Orders n SET ShipPostalCode = ? '
		let mariadb =  await orm.expression(expression).sentence('mariadb', 'northwind')
		mariadb=Helper.replace(mariadb,'\n','; ')
		expect(mariadbExpected).toBe(mariadb)
		const mssqlExpected = 'UPDATE Orders n SET ShipPostalCode = :postalCode '
		let mssql =  await orm.expression(expression).sentence('mssql', 'northwind')
		mssql=Helper.replace(mssql,'\n','; ')
		expect(mssqlExpected).toBe(mssql)
		const mysqlExpected = 'UPDATE Orders n SET ShipPostalCode = ? '
		let mysql =  await orm.expression(expression).sentence('mysql', 'northwind')
		mysql=Helper.replace(mysql,'\n','; ')
		expect(mysqlExpected).toBe(mysql)
		const oracleExpected = 'UPDATE Orders n SET ShipPostalCode = :postalCode '
		let oracle =  await orm.expression(expression).sentence('oracle', 'northwind')
		oracle=Helper.replace(oracle,'\n','; ')
		expect(oracleExpected).toBe(oracle)
		const postgresExpected = 'UPDATE Orders n SET ShipPostalCode = $1 '
		let postgres =  await orm.expression(expression).sentence('postgres', 'northwind')
		postgres=Helper.replace(postgres,'\n','; ')
		expect(postgresExpected).toBe(postgres)
	})
	...
	test('update 5', async () => {
		const expression = 'northwind_1.Orders.update(() => ({ name: entity.name })).include(p => p.details).filter(p => p.id === entity.id)'
		const mariadbExpected = 'UPDATE Orders n SET ShipName = ? WHERE n.OrderID = ? ; UPDATE `Order Details` o SET OrderID = ?,ProductID = ?,UnitPrice = ?,Quantity = ?,Discount = ? WHERE (o.OrderID = ? AND o.ProductID = ?) '
		let mariadb =  await orm.expression(expression).sentence('mariadb', 'northwind')
		mariadb=Helper.replace(mariadb,'\n','; ')
		expect(mariadbExpected).toBe(mariadb)
		const mssqlExpected = 'UPDATE Orders n SET ShipName = :entity.name WHERE n.OrderID = :entity.id ; UPDATE [Order Details] o SET OrderID = :orderId,ProductID = :productId,UnitPrice = :unitPrice,Quantity = :quantity,Discount = :discount WHERE (o.OrderID = :obj.orderId AND o.ProductID = :obj.productId) '
		let mssql =  await orm.expression(expression).sentence('mssql', 'northwind')
		mssql=Helper.replace(mssql,'\n','; ')
		expect(mssqlExpected).toBe(mssql)
		const mysqlExpected = 'UPDATE Orders n SET ShipName = ? WHERE n.OrderID = ? ; UPDATE `Order Details` o SET OrderID = ?,ProductID = ?,UnitPrice = ?,Quantity = ?,Discount = ? WHERE (o.OrderID = ? AND o.ProductID = ?) '
		let mysql =  await orm.expression(expression).sentence('mysql', 'northwind')
		mysql=Helper.replace(mysql,'\n','; ')
		expect(mysqlExpected).toBe(mysql)
		const oracleExpected = 'UPDATE Orders n SET ShipName = :entity.name WHERE n.OrderID = :entity.id ; UPDATE "Order Details" o SET OrderID = :orderId,ProductID = :productId,UnitPrice = :unitPrice,Quantity = :quantity,Discount = :discount WHERE (o.OrderID = :obj.orderId AND o.ProductID = :obj.productId) '
		let oracle =  await orm.expression(expression).sentence('oracle', 'northwind')
		oracle=Helper.replace(oracle,'\n','; ')
		expect(oracleExpected).toBe(oracle)
		const postgresExpected = 'UPDATE Orders n SET ShipName = $1 WHERE n.OrderID = $2 ; UPDATE "Order Details" o SET OrderID = $1,ProductID = $2,UnitPrice = $3,Quantity = $4,Discount = $5 WHERE (o.OrderID = $6 AND o.ProductID = $7) '
		let postgres =  await orm.expression(expression).sentence('postgres', 'northwind')
		postgres=Helper.replace(postgres,'\n','; ')
		expect(postgresExpected).toBe(postgres)
	})
	test('update 6', async () => {
		const expression = 'northwind_1.Orders.update(() => ({ name: entity.name })).include(p => p.details.update(p => p)).filter(p => p.id === entity.id)'
		const mariadbExpected = 'UPDATE Orders n SET ShipName = ? WHERE n.OrderID = ? ; UPDATE `Order Details` o SET UnitPrice = o.UnitPrice,Quantity = o.Quantity,Discount = o.Discount WHERE (o.OrderID = ? AND o.ProductID = ?) '
		let mariadb =  await orm.expression(expression).sentence('mariadb', 'northwind')
		mariadb=Helper.replace(mariadb,'\n','; ')
		expect(mariadbExpected).toBe(mariadb)
		const mssqlExpected = 'UPDATE Orders n SET ShipName = :entity.name WHERE n.OrderID = :entity.id ; UPDATE [Order Details] o SET UnitPrice = o.UnitPrice,Quantity = o.Quantity,Discount = o.Discount WHERE (o.OrderID = :orderId AND o.ProductID = :productId) '
		let mssql =  await orm.expression(expression).sentence('mssql', 'northwind')
		mssql=Helper.replace(mssql,'\n','; ')
		expect(mssqlExpected).toBe(mssql)
		const mysqlExpected = 'UPDATE Orders n SET ShipName = ? WHERE n.OrderID = ? ; UPDATE `Order Details` o SET UnitPrice = o.UnitPrice,Quantity = o.Quantity,Discount = o.Discount WHERE (o.OrderID = ? AND o.ProductID = ?) '
		let mysql =  await orm.expression(expression).sentence('mysql', 'northwind')
		mysql=Helper.replace(mysql,'\n','; ')
		expect(mysqlExpected).toBe(mysql)
		const oracleExpected = 'UPDATE Orders n SET ShipName = :entity.name WHERE n.OrderID = :entity.id ; UPDATE "Order Details" o SET UnitPrice = o.UnitPrice,Quantity = o.Quantity,Discount = o.Discount WHERE (o.OrderID = :orderId AND o.ProductID = :productId) '
		let oracle =  await orm.expression(expression).sentence('oracle', 'northwind')
		oracle=Helper.replace(oracle,'\n','; ')
		expect(oracleExpected).toBe(oracle)
		const postgresExpected = 'UPDATE Orders n SET ShipName = $1 WHERE n.OrderID = $2 ; UPDATE "Order Details" o SET UnitPrice = o.UnitPrice,Quantity = o.Quantity,Discount = o.Discount WHERE (o.OrderID = $1 AND o.ProductID = $2) '
		let postgres =  await orm.expression(expression).sentence('postgres', 'northwind')
		postgres=Helper.replace(postgres,'\n','; ')
		expect(postgresExpected).toBe(postgres)
	})
	test('update 7', async () => {
		const expression = 'northwind_1.Orders.update(() => ({ name: entity.name })).include(p => p.details.update(p => ({ unitPrice: p.unitPrice, productId: p.productId }))).filter(p => p.id === entity.id)'
		const mariadbExpected = 'UPDATE Orders n SET ShipName = ? WHERE n.OrderID = ? ; UPDATE `Order Details` o SET UnitPrice = o.UnitPrice,ProductID = o.ProductID WHERE (o.OrderID = ? AND o.ProductID = ?) '
		let mariadb =  await orm.expression(expression).sentence('mariadb', 'northwind')
		mariadb=Helper.replace(mariadb,'\n','; ')
		expect(mariadbExpected).toBe(mariadb)
		const mssqlExpected = 'UPDATE Orders n SET ShipName = :entity.name WHERE n.OrderID = :entity.id ; UPDATE [Order Details] o SET UnitPrice = o.UnitPrice,ProductID = o.ProductID WHERE (o.OrderID = :orderId AND o.ProductID = :productId) '
		let mssql =  await orm.expression(expression).sentence('mssql', 'northwind')
		mssql=Helper.replace(mssql,'\n','; ')
		expect(mssqlExpected).toBe(mssql)
		const mysqlExpected = 'UPDATE Orders n SET ShipName = ? WHERE n.OrderID = ? ; UPDATE `Order Details` o SET UnitPrice = o.UnitPrice,ProductID = o.ProductID WHERE (o.OrderID = ? AND o.ProductID = ?) '
		let mysql =  await orm.expression(expression).sentence('mysql', 'northwind')
		mysql=Helper.replace(mysql,'\n','; ')
		expect(mysqlExpected).toBe(mysql)
		const oracleExpected = 'UPDATE Orders n SET ShipName = :entity.name WHERE n.OrderID = :entity.id ; UPDATE "Order Details" o SET UnitPrice = o.UnitPrice,ProductID = o.ProductID WHERE (o.OrderID = :orderId AND o.ProductID = :productId) '
		let oracle =  await orm.expression(expression).sentence('oracle', 'northwind')
		oracle=Helper.replace(oracle,'\n','; ')
		expect(oracleExpected).toBe(oracle)
		const postgresExpected = 'UPDATE Orders n SET ShipName = $1 WHERE n.OrderID = $2 ; UPDATE "Order Details" o SET UnitPrice = o.UnitPrice,ProductID = o.ProductID WHERE (o.OrderID = $1 AND o.ProductID = $2) '
		let postgres =  await orm.expression(expression).sentence('postgres', 'northwind')
		postgres=Helper.replace(postgres,'\n','; ')
		expect(postgresExpected).toBe(postgres)
	})
