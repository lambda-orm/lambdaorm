...
describe('Execute', () => {
	...
	test('groupBy 1', async () => {
		...
		const mysqlResult =  await orm.expression(expression).execute(data,'mysql')
		expect(expected).toEqual(mysqlResult)
		const postgresResult =  await orm.expression(expression).execute(data,'postgres')
		expect(expected).toEqual(postgresResult)
		const mariadbResult =  await orm.expression(expression).execute(data,'mariadb')
		expect(expected).toEqual(mariadbResult)
		const mssqlResult =  await orm.expression(expression).execute(data,'mssql')
	...
	test('groupBy 11', async () => {
		...
		const expected = [{"category":"Beverages","largestPrice":263.5},{"category":"Meat/Poultry","largestPrice":123.79},{"category":"Confections","largestPrice":81},{"category":"Seafood","largestPrice":62.5},{"category":"Dairy Products","largestPrice":55},{"category":"Produce","largestPrice":53}]
		const mysqlResult =  await orm.expression(expression).execute(data,'mysql')
		expect(expected).toEqual(mysqlResult)
		const postgresResult =  await orm.expression(expression).execute(data,'postgres')
		expect(expected).toEqual(postgresResult)
