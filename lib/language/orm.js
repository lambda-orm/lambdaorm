...
class OrmSentence extends FunctionRef 
{
    ...
    build(metadata){   
        ...
        let reverse =this._children.find(p=> p.name=='reverse'); 
        ...
        if(map || first){
            ...
            text = select.build(metadata) + ' ' + this.solveFromAndJoins(from,joins,aliases,metadata);
...
class OrmMap extends OrmArrowFunction 
{
    fields(metadata){
        ...
        if(this.children.length==1){
           if(this.children[0] instanceof OrmObject){
               let obj = this.children[0];
               for(let p in obj.children){
                   let keyVal = obj.children[p];
                   fields.push(keyVal.name); 
               }    
           }else if(this.children[0] instanceof OrmArray){
                let array = this.children[0];
           ...
           }else if(this.children[0] instanceof OrmField){
                let parts =this.children[0].name.split('.');
                fields.push(parts[parts.length-1]);
           }
           else{
            fields.push('field0');
...
class OrmMetadata
{
    ...
    getRelation(entity,relation){
        let previousEntity,previousScheme,previousRelation='',relationData,relationEntity,relationScheme;
        ...
        for(let i=0;i<parts.length;i++){
            ...
            if(i==0){
                ...
                previousRelation = previousRelation!=''?previousRelation+'.'+part:part;
            ...
            case 'var':
                ...
                if(parts[0] == context.current.arrowVar){
                    if(parts.length > 2){
                        ...
                        let entity = new OrmName(context.current.entity,this._name);
                        return new OrmField(context.current.alias+'.'+parts[1],this._name,[entity]);      
                    }
                }
                else
