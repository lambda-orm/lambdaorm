...
class OrmSentence extends FunctionRef 
{
    ...
    solveJoins(joins,metadata){
        ...
        for(let i=0;i<joins.length;i++){
            let join = joins[i];
            let joinText  = template.replace('{name}',join.name);         
            joinText =joinText.replace('{alias}',join.children[0].name);
            joinText =joinText.replace('{relation}',join.children[1].build(metadata));           
            text= text + joinText+'\r\n';
        }
        return text;
    }
    solveFrom(from,metadata){
        ...
        template =template.replace('{name}',from.name); 
    ...
    nodeToOperand(node,scheme,context){
        ...
        if(node.type == 'arrow'){
            ...
            if(sentence['from']){
                ...
                operand =new OrmFrom(tableName,this._name,[alias]);
            ...
            for(const key in context.current.joins){
                ...
                let alias = new OrmAlias(relationAlias,this._name);
                ...
                operand = new OrmJoin(relationTable,this._name,[alias,equal]);
                children.push(operand);   
            }
