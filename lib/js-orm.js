...
class SourceManager
{
    ...
    addLibrary(library){
        ...
        for(const name in library.operators){
            let data= library.operators[name]
            this._model.addOperator(name,data.cardinality,data.metadata)
        }
    ...
    constructor(model){
     ...
     refresh(){
         for(const key in this._model.operators){
             if( key.length==2) this._doubleOperators.append(key);
             else if(key.length==3) this._tripleOperators.append(key);
         ...
         if(char.isalnum()){    
             ...
             if(value=='if' && this.current == '('){
                 ...
                 if(value.includes('.')){
                     ...
                     let variable = Node(variableName,'variable');
