...
class Model
{
    ...
    getOperatorMetadata(name,cardinality){
        try{          
            ...
            throw ModelException('error with operator: '+name,error);
        }
    } 
    getFunctionMetadata(name){
        try{
            ...
            throw ModelException('error with function: '+name,error);
        }
    }
} 
class SourceManager
...
class Exp {
    ...
    parse(expression){
        try{
            ...
            throw ModelException('expression: '+expression+' error: '+error.toString());
        }
    }
}
